...
/* ─── 2026/27 SARS DATA (Tax Year 2027: 1 Mar 2026 – 28 Feb 2027) ─── */
const TAX_DATA = {
  label: "2026/27",
  ...
  // Medical scheme fees tax credit (per month) — SARS 2025/26 rates
  medCredits: { main: 364, first: 364, additional: 246 },
  uifRate: 0.01,
  uifMonthCap: 17712 / 12, // R17,712 annual cap
};
...
function calculate(
  ...
  // RA/pension deduction — max 27.5% of income or R350,000
...
export default function TaxCalculator() {
  ...
  const result = useMemo(
    () => calculate(incomeNum, age, medMembers, raNum),
    [incomeNum, age, medMembers, raNum]
  );

  const hasIncome = incomeNum > 0;

  function findBracket(income: number) {
    return TAX_DATA.brackets.find(
      (b) => income >= b.min && income <= b.max
    );
  }
  const activeBracket = hasIncome ? findBracket(result.taxableIncome) : null;
    ...
    <div>
      <div className="grid lg:grid-cols-5 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="card p-6 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["under65", "65-74", "75+"] as AgeGroup[]).map((a) => (
                  <button
                    key={a}
                    onClick={() => { setAge(a); setCalculated(true); }}
            ...
            <div>
              ...
              <select
                value={medMembers}
                onChange={(e) => { setMedMembers(Number(e.target.value)); setCalculated(true); }}
            ...
            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1.5">
                Annual RA / pension fund contributions{" "}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  inputMode="numeric"
                  value={raContrib}
                  onChange={(e) => { setRaContrib(e.target.value); setCalculated(true); }}
                  placeholder="0"
                  className="w-full pl-8 pr-4 py-2.5 border border-neutral-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand transition-colors"
                />
              </div>
        ...
        <div className="lg:col-span-3">
          {!hasIncome ? (
            <div className="card p-10 text-center text-neutral-400 flex flex-col items-center gap-3">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl bg-brand-dark text-white p-6">
                ...
                <p className="text-xs text-brand-100">
                  {fmt(result.annualTakeHome)} per year · after tax &amp; UIF
                </p>
                ...
                {result.belowThreshold && (
                  <div className="mt-3 flex items-start gap-2 bg-white/10 rounded-lg px-3 py-2">
                    ...
                    <p className="text-xs text-brand-100">
                      Your income falls below the tax threshold — no income tax is payable.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-start gap-2 text-2xs text-neutral-400 px-1">
                ...
                <span>
                  Estimates only. Does not account for fringe benefits, additional deductions, or
                  allowances. Consult a{" "}
